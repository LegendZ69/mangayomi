...
class _TranslationScreenState extends State<TranslationScreen> {
  ...
  Future<void> _importPage() async {
    ...
    try {
      ...
      if (file.size <= 0 || file.size > maxBytes) {
        _message(
          'Choose a nonempty image no larger than 7 MB (7,000,000 bytes).',
        );
        return;
      }
    ...
    switch (action) {
      case 'preview':
        ...
        await Navigator.of(context).push(
          MaterialPageRoute<void>(
            builder: (_) =>
                TranslationPreview(job: job, controller: _controller),
          ),
        );
      case 'retry':
        _controller.pause();
      ...
      case 'requeue':
        final copy = await showDialog<bool>(
          ...
          builder: (context) => AlertDialog(
            ...
            actions: [
              ...
              FilledButton(
                onPressed: () => Navigator.pop(context, true),
                child: const Text('Queue new copy'),
              ),
            ],
          ),
        );
        ...
        _message(
          added == null
              ? (_controller.error ??
                    'Could not queue a new copy of this page.')
  ...
  Widget _buildQueue(BuildContext context) {
    ...
    jobs.sort(
      (a, b) => switch (_sort) {
        ...
        _QueueSort.name => a.title.toLowerCase().compareTo(
          b.title.toLowerCase(),
        ),
  ...
  Widget _jobTile(TranslationJob job) {
    ...
    final color = switch (job.status) {
      TranslationJobStatus.failed ||
      TranslationJobStatus.interrupted => Theme.of(context).colorScheme.error,
        ...
        trailing: PopupMenuButton<String>(
          ...
          itemBuilder: (_) => [
            ...
            PopupMenuItem(
              ...
              child: ListTile(
                leading: Icon(
                  active ? Icons.cancel_outlined : Icons.delete_outline,
                ),
  ...
  Widget _buildLogs(BuildContext context) {
    ...
    final logs = allLogs.where((event) {
      return (selectedPhase == null || event.phase == selectedPhase) &&
          (query.isEmpty ||
              jsonEncode(event.toJson()).toLowerCase().contains(query));
    }).toList();
    logs.sort(
      (a, b) => _newestLogsFirst
              ...
              children: [
                ...
                Row(
                  children: [
                    ...
                    IconButton(
                      tooltip: _newestLogsFirst
                          ? 'Show oldest first'
                          : 'Show newest first',
                      onPressed: () =>
                          setState(() => _newestLogsFirst = !_newestLogsFirst),
                      icon: Icon(
                        _newestLogsFirst
        ...
        if (logs.isEmpty)
          const SliverToBoxAdapter(
            child: _EmptyState(
              ...
              message:
                  'Queue actions, settings, timing, and API outcomes '
  ...
  Future<void> _logAction(String action) async {
    if (action == 'clear') {
      final clear = await showDialog<bool>(
        ...
        builder: (context) => AlertDialog(
          ...
          content: const Text(
            'This removes the saved diagnostic history and '
    ...
    final approved = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text(
          'Credentials are redacted, but logs may include page '
    ...
    try {
      if (action == 'copy') {
        ...
        await shareOrCopy(
          ShareParams(
            files: [
              XFile.fromData(utf8.encode(logs), mimeType: 'application/json'),
            ],
...
class _LogTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final raw = const JsonEncoder.withIndent('  ')
        .convert(redactTranslationData(event.toJson()));
          ...
          onPressed: () async {
            ...
            if (context.mounted) {
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(
                  content: Text(
                    'Redacted event copied; it may contain page text.',
                  ),
                ),
              );
            }
