...
class TranslationSettingsPanel extends StatefulWidget {
  ...
  @override
  State<TranslationSettingsPanel> createState() => _TranslationSettingsPanelState();
}
...
class _TranslationSettingsPanelState extends State<TranslationSettingsPanel>
  ...
  void _loadDraft(TranslationSettings settings) {
    ...
    final values = <String, String>{
      ...
      'rawGenerationConfig': const JsonEncoder.withIndent('  ').convert(settings.rawGenerationConfig),
  ...
  TranslationSettings _readDraft() {
    ...
    if (raw.length > 32768) {
      throw const FormatException('Raw generation JSON exceeds the 32,768-character app safety limit.');
    }
    final decoded = raw.isEmpty ? <String, dynamic>{} : jsonDecode(raw);
    if (decoded is! Map<String, dynamic>) {
      throw const FormatException('Raw generation configuration must be a JSON object.');
    }
    final fontSize = _text('fontSize').isEmpty ? null : double.tryParse(_text('fontSize'));
    if (_text('fontSize').isNotEmpty && fontSize == null) {
      throw const FormatException('Font size must be a number, or blank for system default.');
    }
    return _draft.copyWith(
      ...
      maxOutputTokens: _optionalInteger('maxOutputTokens', 'Maximum output tokens'),
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      if (saved) {
        _draft = settings;
        _report('Settings saved for newly queued pages. Existing jobs keep their snapshot.');
      } else {
        _report(widget.controller.error ?? 'Settings could not be saved.', error: true);
      }
    } on FormatException catch (error) {
      _report(error.message, error: true);
    } catch (_) {
      _report('Settings could not be saved. Check the values and try again.', error: true);
    } finally {
  ...
  Future<void> _saveCredential({bool remove = false}) async {
    ...
    if (remove) {
      final confirmed = await showDialog<bool>(
        ...
        builder: (context) => AlertDialog(
          ...
          content: const Text('Queued jobs using this Vertex mode will need a new credential before they can run.'),
          actions: [
    ...
    try {
      ...
      if (saved) {
        ...
        _report(remove ? 'Saved credential removed.' : 'Credential saved securely. It is never included in logs.');
      } else {
        _report('The credential could not be saved securely.', error: true);
      }
    } catch (_) {
      _report('Secure credential storage is unavailable. No credential was logged.', error: true);
    } finally {
      ...
      final accepted = await showDialog<bool>(
        ...
        builder: (context) => AlertDialog(
          ...
          content: const SingleChildScrollView(
            child: Text('Raw diagnostic bodies may include source text, translations, '
          ...
          actions: [
            TextButton(onPressed: () => Navigator.pop(context, false), child: const Text('Keep off')),
            FilledButton(onPressed: () => Navigator.pop(context, true), child: const Text('Enable raw capture')),
          ],
        ),
      );
  ...
  Widget build(BuildContext context) {
    ...
    return ListView(
      ...
      children: [
        ...
        const Text('Defaults: hybrid vision execution, full Vertex AI, global, '
        ...
        _section(
          ...
          children: [
            _picker<ExecutionMode>(
              ...
              onChanged: (value) => setState(() => _draft = _draft.copyWith(executionMode: value)),
            ),
            const _SettingsNotice(
              icon: Icons.cloud_upload_outlined,
              text: 'Local only applies to PP-OCR / YOLO26 / LaMa. Selected AI '
            ...
            _picker<DetectionEngine>(
              ...
              onChanged: (value) => setState(() => _draft = _draft.copyWith(detectionEngine: value)),
            ),
            _picker<OcrEngine>(
              label: 'OCR',
              ...
              onChanged: (value) => setState(() => _draft = _draft.copyWith(ocrEngine: value)),
            ),
            _picker<InpaintEngine>(
              ...
              onChanged: (value) => setState(() => _draft = _draft.copyWith(inpaintEngine: value)),
            ),
            if (unsupported.isNotEmpty)
              _SettingsNotice(
                icon: Icons.warning_amber_outlined,
                text: 'Configuration only — cannot run this pipeline:\n${unsupported.join('\n')}',
              ),
            const Text('This build executes AI detection + OCR + translation in '
            ...
            const SizedBox(height: 16),
            _modelSelector('ppocrDetectionModel', 'PP-OCR detection model', _ppocrDetectionModels),
            _modelSelector('ppocrRecognitionModel', 'PP-OCR recognition model', _ppocrRecognitionModels),
            const Text('PP-OCRv6 medium/small include Japanese; v6 tiny does '
            ...
            const Text('YOLO26’s ordinary COCO checkpoints do not detect manga '
        ...
        _section(
          ...
          children: [
            ...
            _field('projectId', 'Google Cloud project ID',
            ...
            _field('location', 'Location', helper: 'Default: global. Model availability varies by location.'),
            _field('model', 'Model ID', helper: 'This adapter currently validates only gemini-3.8-flash. Other IDs fail explicitly; no silent substitution.'),
            if (_draft.vertexMode == VertexMode.express)
              const _SettingsNotice(
                icon: Icons.warning_amber_outlined,
                text: 'Gemini 3.8 Flash is not listed in Google’s Express model '
            ...
            Wrap(
              ...
              children: [
                OutlinedButton.icon(
                  ...
                  label: Text(_savingCredential ? 'Saving…' : 'Save credential'),
                ),
                TextButton.icon(
            ...
            SwitchListTile.adaptive(
              ...
              subtitle: const Text('Off by default. Requires provider support; '
                  'this toggle does not buy or provision capacity.'),
              value: _draft.priorityPaygo,
              onChanged: (value) => setState(() => _draft = _draft.copyWith(priorityPaygo: value)),
            ),
            SwitchListTile.adaptive(
              contentPadding: EdgeInsets.zero,
              title: const Text('Provisioned Throughput'),
              subtitle: const Text('Off by default. Requires separately purchased '
                  'capacity. Unsupported combinations fail explicitly.'),
              value: _draft.provisionedThroughput,
              onChanged: (value) => setState(() => _draft = _draft.copyWith(provisionedThroughput: value)),
            ),
          ],
        ),
        _section(
          ...
          children: [
            _field('targetLanguage', 'Target language', helper: 'App preference: English. Any unambiguous language name can be requested.'),
            _picker<String>(
              label: 'Thinking level',
              value: _draft.thinkingLevel,
              labels: {
                if (_draft.thinkingLevel == 'MINIMAL') 'MINIMAL': 'Minimal — unsupported for 3.8',
                'LOW': 'Low', 'MEDIUM': 'Medium (provider default)', 'HIGH': 'High',
              },
              onChanged: (value) => setState(() => _draft = _draft.copyWith(thinkingLevel: value)),
            ),
            _field('maxOutputTokens', 'Maximum output tokens',
              numeric: true,
              helper: 'Blank omits the field and uses the provider default. The API enforces model-specific limits.'),
            const _SettingsNotice(
              icon: Icons.info_outline,
              text: 'Gemini 3.8 ignores temperature, topP, and topK. Legacy '
            ...
            TextField(
              ...
              decoration: const InputDecoration(
                ...
                helperText: 'JSON object only; no credentials, URL, headers, or '
        ...
        _section(
          ...
          children: [
            const Text('These are app policies, not Google model defaults. '
                'Jobs run sequentially to limit iPhone memory and request bursts.'),
            const SizedBox(height: 16),
            _field('timeoutSeconds', 'Request timeout (seconds)', numeric: true, helper: 'App default: 60. A timed-out request may still have incurred provider charges.'),
            _field('maxRetries', 'Maximum automatic retries', numeric: true, helper: 'App default: 2. Transient failures only; retried requests may incur charges.'),
            _field('maxLogEntries', 'Retained log events', numeric: true, helper: 'App default: 500. App safety range: 1–10,000; older events are removed.'),
            SwitchListTile.adaptive(
              contentPadding: EdgeInsets.zero,
              title: const Text('Capture raw request / response bodies'),
              subtitle: const Text('Off by default. Includes private page text; '
        ...
        _section(
          ...
          children: [
            const Text('Blank values inherit the system theme’s font, size, '
            ...
            const SizedBox(height: 16),
            _field('fontFamily', 'Font family override', helper: 'Blank: system theme. A named font must already be installed or bundled.'),
            _field('fontSize', 'Font size override', numeric: true, decimal: true, helper: 'Blank: system theme. Positive logical pixels before region fitting.'),
            _field('fontWeight', 'Font weight override', numeric: true, helper: 'Blank: system theme. 100–900 in steps of 100.'),
            _field('backgroundColor', 'Overlay background color', helper: 'Blank: system surface. #RRGGBB or #AARRGGBB.'),
            _colorPresets('backgroundColor'),
            _field('textColor', 'Overlay text color', helper: 'Blank: system text. #RRGGBB or #AARRGGBB.'),
            _colorPresets('textColor'),
            OutlinedButton.icon(
              onPressed: () {
                for (final key in ['fontFamily', 'fontSize', 'fontWeight', 'backgroundColor', 'textColor']) {
                  _fields[key]!.clear();
                }
        ...
        _section(
          ...
          children: [
            const Text('Model/API constraints come from official documentation. '
                'App defaults and safety caps are labelled separately. '
                'A listed model variant does not imply an installed engine.'),
            _reference('Gemini 3.8 Flash model guide', 'https://docs.cloud.google.com/gemini-enterprise-agent-platform/models/guides/gemini-3-8-flash'),
            _reference('Vertex AI Express supported models', 'https://docs.cloud.google.com/gemini-enterprise-agent-platform/models/start/express-mode/overview'),
            _reference('PaddleOCR iOS deployment', 'https://www.paddleocr.ai/main/en/version3.x/inference_deployment/cross_platform/ios_deployment.html'),
            _reference('Ultralytics YOLO26', 'https://docs.ultralytics.com/models/yolo26/'),
          ],
        ),
        const SizedBox(height: 16),
        FilledButton.icon(
  ...
  Widget _section({required String title, required IconData icon,
      required List<Widget> children, bool initiallyExpanded = false}) => Card(
  ...
  Widget _picker<T extends Object>({required String label, required T value,
      required Map<T, String> labels, required ValueChanged<T> onChanged}) => Padding(
    ...
    child: DropdownButtonFormField<T>(
      ...
      items: labels.entries.map((entry) => DropdownMenuItem(
        value: entry.key,
        child: Text(entry.value, maxLines: 2, overflow: TextOverflow.ellipsis),
      )).toList(),
      onChanged: (selected) { if (selected != null) onChanged(selected); },
    ),
  );

  Widget _field(String key, String label, {String? helper, bool numeric = false, bool decimal = false}) => Padding(
    padding: const EdgeInsets.only(bottom: 16),
    child: TextField(
      ...
      inputFormatters: numeric && !decimal ? [FilteringTextInputFormatter.digitsOnly] : null,
      decoration: InputDecoration(
        labelText: label,
        helperText: helper,
        helperMaxLines: 5,
  ...
  Widget _colorPresets(String field) => Padding(
    ...
    child: Wrap(
      ...
      children: [
        ...
        for (final entry in const {'Black': 0xff000000, 'White': 0xffffffff, 'Cream': 0xfffff3d6}.entries)
          ActionChip(
  ...
  Widget _modelSelector(String field, String label, List<String> models) => Padding(
    padding: const EdgeInsets.only(bottom: 16),
    child: Column(
      crossAxisAlignment: CrossAxisAlignment.stretch,
      children: [
        _field(field, label, helper: 'Configuration only — adapter unavailable. '
            'Enter an exact artifact/model ID or choose an upstream catalog entry.'),
        OutlinedButton.icon(
          onPressed: () async {
            final selected = await showDialog<String>(
              context: context,
              builder: (_) => _ModelCatalogDialog(title: label, models: models),
            );
            if (selected != null && mounted) _fields[field]!.text = selected;
          },
...
const _ppocrRecognitionModels = [
  ...
  'korean_PP-OCRv5_mobile_rec', 'latin_PP-OCRv5_mobile_rec',
  'eslav_PP-OCRv5_mobile_rec', 'th_PP-OCRv5_mobile_rec', 'el_PP-OCRv5_mobile_rec',
  'arabic_PP-OCRv5_mobile_rec', 'cyrillic_PP-OCRv5_mobile_rec',
  'devanagari_PP-OCRv5_mobile_rec', 'te_PP-OCRv5_mobile_rec', 'ta_PP-OCRv5_mobile_rec',
  'PP-OCRv4_server_rec_doc', 'PP-OCRv4_mobile_rec', 'PP-OCRv4_server_rec',
  'en_PP-OCRv4_mobile_rec', 'PP-OCRv3_mobile_rec', 'en_PP-OCRv3_mobile_rec',
  'korean_PP-OCRv3_mobile_rec', 'japan_PP-OCRv3_mobile_rec',
  'chinese_cht_PP-OCRv3_mobile_rec', 'te_PP-OCRv3_mobile_rec',
  'ka_PP-OCRv3_mobile_rec', 'ta_PP-OCRv3_mobile_rec', 'latin_PP-OCRv3_mobile_rec',
  'arabic_PP-OCRv3_mobile_rec', 'cyrillic_PP-OCRv3_mobile_rec',
...
class _SettingsNotice extends StatelessWidget {
  const _SettingsNotice({required this.icon, required this.text, this.isError = false});
  final IconData icon;
  final String text;
  final bool isError;
  ...
  Widget build(BuildContext context) => Container(
    ...
    decoration: BoxDecoration(
      color: isError ? Theme.of(context).colorScheme.errorContainer
