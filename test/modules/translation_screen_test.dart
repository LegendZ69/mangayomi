...
void main() {
  ...
  testWidgets(
    'iPhone 13 queue has labelled actions and no large-text overflow',
    (tester) async {
  ...
  testWidgets('vision execution options do not imply offline Gemini', (
    tester,
  ) async {
    await _mount(tester, controller, textScale: 1.6);
    ...
    expect(
      find.textContaining('Local only applies to PP-OCR / YOLO26 / LaMa.'),
      findsOneWidget,
    );
    ...
    expect(
      find.textContaining('Gemini translation is always cloud-based'),
      findsOneWidget,
    );
    expect(tester.takeException(), isNull);
    expect(clientCreations, 0);
  ...
  testWidgets(
    'Run requires explicit upload consent and cancellation sends nothing',
    (tester) async {
      await controller.initialize();
  ...
  testWidgets('queue and log headers scroll with a keyboard and in landscape', (
    tester,
  ) async {
    await _mount(tester, controller, textScale: 1.6);
  ...
  testWidgets(
    'model catalog remains scrollable with large text and a keyboard',
    (tester) async {
      await _mount(tester, controller, textScale: 1.6);
      await tester.tap(find.widgetWithText(Tab, 'Settings'));
      await tester.pumpAndSettle();
